      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/10">
          {data.programs.map((program, i) => (
            <Link key={program._id} href={`/programs/${program.slug.current}`} className="group relative overflow-hidden bg-dark">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <p className="text-primary text-xs font-semibold tracking-wider uppercase mb-1">{program.degree}</p>
                <h3 className="font-serif text-xl text-white mb-2">{program.title}</h3>
                <p className="text-white/60 text-xs leading-relaxed line-clamp-2 mb-4">{program.description}</p>
